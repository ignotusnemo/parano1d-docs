...
| `--p2p-listen HOST:PORT` | Адрес приёма P2P-соединений; по умолчанию `0.0.0.0:9400` |
| `--rpc-listen HOST:PORT` | Адрес приёма JSON-RPC-соединений; по умолчанию `127.0.0.1:9401` |
...
аутентифицированным транспортом. Токен доступа аутентифицирует запросы, но не
шифрует их.
...
|---|---|---|
| `--rpc URL` | `http://127.0.0.1:9401` | Адрес JSON-RPC ноды |
...
Стандартный адрес — `http://127.0.0.1:9401`. Его можно изменить переменной
...
|---|---|---|
| `status` | — | Вершина, хеш, сложность и сводка Live State |
