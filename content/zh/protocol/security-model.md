...
实际部署配置对应两项不同的安全性命题。Block–Tiwari 衡量 FRI 在经典
...
其中最小值遍历所有正整数查询预算。对实际部署的 B64 与 B255 配置应用其定义、
...
安全游戏考察单个有状态量子对手能否使实际部署的验证器接受一个递归谱系始于
创世区块的无效终端 State。同一资源预算覆盖钱包授权、区块关系、父链接、精确
State 转换、递归验证，以及终端证明依赖的全部对手构造祖先。

`C1` 是源码中实际部署宽挑战配置的标识符。该配置使用 65 次钱包查询、133 次
History 查询、256 位交互记录摘要，并从 `GF(2^256)` 中基数为 `2^255` 的
迹为 1 的仿射集合均匀采样代数挑战。已承诺执行轨迹与 Poseidon2b 的算术仍运行在
`GF(2^128)` 上。

深度感知定理针对 AES-128 的 `2^170` gate-depth 参考值，检查 NIST 后量子
密码学 Category 1 的全部 `MAXDEPTH` 点。主导半成功 gate-depth 下界的以二为底
对数为 `173.273866314232` 位，Category 1 资源包络下完整理想成功概率上界
不超过 `0.053364140323608411`。

固定 Poseidon2b 实际部署推论要求
`Delta_P2b^C1 < 0.446635859676391589`，并采用资源定理声明的最小相干响应成本
前提。在这些前提下，Category 1 资源包络内任意对手在从创世开始的无效 State
游戏中，成功概率均小于二分之一。因此，网络当前状态自创世块起的端到端后量子
可靠性，已证明达到 NIST PQC Category 1 水平。

[端到端 QROM 推导](https://github.com/ignotusnemo/parano1d/blob/main/noid_soundness/docs/category-one.md)给出安全游戏、
归约、有限项和全部前提。[`noid_soundness` 证书](https://github.com/ignotusnemo/parano1d/tree/main/noid_soundness)
从实际部署代码导入常量，并用精确整数或有理数算术计算所有规范性不等式。这是
