...
默认 P2P 监听 TCP `9400`。[JSON-RPC](../reference/glossary.md#rpc) 是单独的本地管理接口，监听 `127.0.0.1:9401`。
...
[GossipSub](../reference/glossary.md#gossipsub) 传输[交易意图](../reference/glossary.md#transaction-intent)和区块公告。
...
- 批量区块头；
- 保留的完整区块；
- 快照清单与 [State 分段](../reference/glossary.md#state-segment)；
- 近期内存池清单与缺失的交易意图。
...
普通挖矿要求至少连接两个[已完成身份认证的对等节点](../reference/glossary.md#authenticated-peer)。这可以防止无人值守的节点仅因网络链路中断，就继续扩展其隔离的私有视图。
